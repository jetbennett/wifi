...
#returns a time 0 to 15
#Time a node waits if it detects a busy medium before collisions
def busyWait():
  return randint(0, 15)

def binExpBackoff(time, numOfBackoffs):
  return randint(0, 2 ** numOfBackoffs)
...
with open(outPath, 'w') as of:
  with open(trafficfile, 'r') as tf:
    ...
    for line in tf:
      ...
      numSuccess = processQue(sending_qwee, time, numSuccess)
